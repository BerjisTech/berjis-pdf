...
func New(opts Options) *fiber.App {
	...
	if coreAPIBase != "" {
		if v, err := coreauth.NewVerifier(coreauth.Config{
			...
			fmt.Printf("warn: pdf coreauth verifier init failed: %v\n", err)
		} else {
			authVerifier = v
		}
	}
		...
		if token != "" && authVerifier != nil {
			if claims, err := authVerifier.Verify(token); err == nil {
				...
				if !errors.Is(err, coreauth.ErrJWKSUnavailable) {
					fmt.Printf("warn: pdf coreauth verify failed: %v\n", err)
				}
			}
		}
	...
	setStatus := func(c *fiber.Ctx, status string) error {
        ...
        user_id=$3 OR EXISTS(SELECT 1 FROM pdf_collaborators pc WHERE pc.pdf_id=$2 AND pc.user_id=$3 AND pc.role='editor')
      )
	...
	app.Post("/v1/pdfs/:id/archive", func(c *fiber.Ctx) error { return setStatus(c, "archived") })
	app.Post("/v1/pdfs/:id/restore", func(c *fiber.Ctx) error { return setStatus(c, "active") })
	app.Delete("/v1/pdfs/:id", func(c *fiber.Ctx) error { return setStatus(c, "deleted") })
